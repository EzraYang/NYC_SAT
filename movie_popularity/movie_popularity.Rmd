...
As a reminder the boss's goal is to find out what attributes make a movie popular, as well as anything new about movies. So we can raise questions in a vast range, as long as to do some pre-processing for later modeling.
...
Just for curiosity, if we slice the data by the quantiles of imdb_num_votes and look at their relationship with the 3 numerical scoring variable, will there be significant difference by eye between groups?  Instinctively, one might say that the movie with phenomenal number of votes should win a higher score, for its high quality attracts so many people to watch it and rank it. Let's see what the data have to say.  
```{r}
...
There seems to be a slight curvilinear relationship between quantiles of number of votes and each of the 3 scoring variable. Movies with fewest and highest number of votes get a better score compared to the rest two groups.   
...
Here's the `grand_sc` variable, it is bimodeled and has a moderate left skewness,  as is reasonable inferred from its 3 ingredient variables .
...
# this is the first 6 variables of type Factor
grid.arrange(fac1, fac2, fac3, fac4, fac5, fac6, ncol = 3)
```
```{r}
# the rest 6 variables of type Factor
...
(Don't panic at these terms, correlation coeficient value simply tells how strong the relationship is between the two variable you are examing, and here p value is just the probability that the two variable of interest don't have any relationship at all. )  
```{r}
...
The rest two, `runtime` and `imdb_num_votes` both provide weak correlation coefficient r with `grand_sc`.
...
# extract the adjusted R squared value of each model
ldply(models1, function(x) summary(x)$adj.r.squared)
```

The 9th model has highest adjusted $R^2$ value, so m9 is the new full model, the first variable to be excluded is `top200_box`.  
Now, we have 8 explaining variables at hand. Based on m9 as full model, I'm going to exclude one variable at a time, resulting in 8 new different models.
...
Remember the new full model m9 has a adjusted $R^2$ value of 0.3992460, after eliminating a second variable, no model grants a higher adjusted $R^2$ value than it.  
...
* if $x$ is one level of a categorical variable, else hold constant, when $x=1$, the expected grand score is increased by $\beta$.  
...
Now we can have a idea of what's the most popular movie:  
A Documentary of genre "Musical & Performing Arts", with a "G" as its MPAA rating, runs a long time, have many many IMDB votes, the director of which won an Oscar ever, and gets nominated for a best picture Oscar but didn't make it through the final winner.  
...
So first I fetch 3 scoring variables from [IMDB   ](http://www.imdb.com/title/tt3783958/?ref_=fn_al_tt_1) and [Rotten Tomatoes](https://www.rottentomatoes.com/m/la_la_land) page, and calculate the true `grand_sc`, which is 85.5. The calculation is at below.    
```{r}
...
* Though I like my work, there's some innegligible shortcomings that I have to state.  
    ...
    * There is a "Documentary" level in both `title_type` and `genre`. Future studies could optimize the class of a movie so there could be minimum redundant information storing in 2 different variables.
